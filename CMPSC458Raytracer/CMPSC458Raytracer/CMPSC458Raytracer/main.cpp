...
void drawScene(int d)
{
	...
	float t = dir.Length() * tan(fovy/2.0);
	float r = dir.Length() * tan(fovx/2.0);
	float l = -r;
	float b = -t;
	cout << "dir: " << dir << "\n";
	cout << "t: " << t << ", r: " << r << "\n";
	...
	//for (int x=75; x < 175; x ++)
	{
		...
		//for (int y=200; y < 300; y++)
		{
			...
			float leftness	= r + ( 2.0 * l ) * ((x+0.5)/float(WIDTH));
			float upness	= t + ( 2.0 * b ) * ((y+0.5)/float(HEIGHT));
			currentDir = (left * leftness) + (up * upness) + dir;
